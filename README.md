...
4. 用自然语言填写任务内容，再选择企业微信、邮件或两种推送方式并填写接收目标。
5. 默认的 MCP 配置路径、允许服务、超时和执行轮数收在“高级设置”中。工程配置或 MCP 服务器名称不同时再展开修改，然后保存；已有任务也可以点击“立即运行”验证结果。

企业微信群的 `chatid` 可以通过在目标群中 @机器人并发送 `/chatid` 查询。需要允许群成员远程触发任务时，在“高级设置”中配置用户白名单。配置好目标群和用户白名单后，可以引用带 `[RPT-…]` 的机器人报告继续提问，也可以发送 `/run 任务名称`。如果一个群对当前用户只开放了一个任务，普通 @提问也会路由到该任务。用户白名单留空时禁止群内触发，只有明确填写 `*` 才允许群内所有成员。
...
- 自动化任务和最近 500 条运行记录独立保存在 `%APPDATA%\Claude Workspace\automation.json`。其中包含提示词、收件人地址、企业微信目标与用户 ID、结果和诊断信息，但不保存 MCP 配置内容。
