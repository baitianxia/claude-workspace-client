...
- 每条企业微信待回复消息使用独立回复码，并绑定客户端会话和本次 Claude Code 进程；多个进程并发等待时不会按“最近消息”模糊路由。
...
4. 收到通知后，可以直接引用该消息并只发送回复内容；不引用时按消息给出的格式回复，例如 `AB3KD7Q2 1` 或 `AB3KD7Q2 继续处理并运行测试`。多个问题之间使用分号，多选项使用逗号，例如 `AB3KD7Q2 1;2,3`。

`AskUserQuestion` 会按 Claude Code 的问答界面展示原问题、选项说明以及自动追加的“输入其他回答（Type something.）”和“与 Claude 讨论这个问题（Chat about this）”，不会展示 Hook 内部 JSON。单选和多选问题都可回复编号或完整选项文字；权限菜单也可回复编号、`允许`/`拒绝`或通知中展示的完整选项文字。文字只做全半角、大小写、连续空格及末尾 `(Esc)` 的最小规范化，且必须唯一、完整匹配；不使用包含、模糊相似度或 AI 猜测，存在歧义时会要求改用编号。菜单文字匹配后才会写入方向键和 Enter，不会作为普通提示词发送。选择“输入其他回答”“与 Claude 讨论这个问题”或权限拒绝后，客户端会保留同一个回复码，并提示继续发送文字；该阶段的内容才会原样写入 Claude Code 输入框。
...
当前每个客户端配置一个接收用户 userid，主动通知只推送给该用户；入站侧接受机器人单聊中带有当前有效回复码的消息，群聊消息不会写入终端。客户端退出后长连接与本地 Claude Code 进程都会结束，因此远程回复功能也会停止。

企业微信主动推送使用的通讯录 userid 与消息回调中的机器人作用域 userid 可能不同，因此客户端不比较两者。8 位随机回复码是入站授权凭证；任何持有当前有效回复码的人都能向对应 Claude Code 进程发送输入，请勿转发带有回复码的通知。

左侧企业微信状态卡会显示最近一条入站回复的处理结果，包括已路由、回复码无效或消息被忽略。若企业微信里发出回复后这里始终没有入站记录，说明回复没有到达当前客户端，应优先检查机器人长连接是否被其他客户端占用。
...
- 企业微信回复必须匹配唯一回复码；过期码、已退出/重启进程的码不会写入任何终端。
