...
`AskUserQuestion` 会按 Claude Code 的问答界面展示原问题、选项说明以及自动追加的“输入其他回答（Type something.）”和“与 Claude 讨论这个问题（Chat about this）”，不会展示 Hook 内部 JSON。单选问题可回复编号或完整选项文字；选择后两项时，客户端会保留同一个回复码，并提示继续发送自定义答案或讨论内容。权限拒绝后需要说明调整要求时也使用相同的两阶段回复方式。
