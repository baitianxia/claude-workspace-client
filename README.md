...
- 可连接企业微信“智能机器人”的 API 长连接模式；Claude Code 请求权限、提出问题、等待计划确认或等待下一步输入时，主动推送到指定企业微信用户并接收远程回复。
...
[`PermissionRequest`、`PreToolUse` 和 `Notification` Hooks](https://code.claude.com/docs/en/hooks)
...
4. 收到通知后按消息给出的格式回复，例如 `AB3KD 1` 或 `AB3KD 继续处理并运行测试`。多个问题之间使用分号，多选项使用逗号，例如 `AB3KD 1;2,3`。

回复码是强制的。即使当时看起来只有一个待回复会话，客户端也不会把无回复码消息自动发给“最近的”进程，因为用户阅读消息和发送回复之间可能又出现新的并发请求。引用一条包含回复码的机器人通知时，也可以只发送回复内容。
...
- 为便于远程判断，通知会包含待确认工具、命令或路径以及 Claude Code 的问题内容；请限制机器人的可见范围，并避免把凭据直接写入命令或问题文本。
