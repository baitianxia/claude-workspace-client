...
4. 用自然语言填写任务内容，再选择企业微信、邮件或两种推送方式。第一次选择企业微信时，点击“管理机器人”添加一个自动化推送机器人，填写本机名称、Bot ID 和 Secret；可以添加多个机器人，所有已启用项都会各自保持在线。
5. 为当前任务固定选择一个自动化推送机器人，再从这个机器人发现的群中选择接收目标。多个任务可以共享同一个机器人，也可以分别使用不同机器人；个人接收人或未发现的群仍可手动填写 `userid` 或 `chatid`。
6. 默认的 MCP 配置路径、允许服务、超时和执行轮数收在“高级设置”中。工程配置或 MCP 服务器名称不同时再展开修改，然后保存；已有任务也可以点击“立即运行”验证结果。

目标群首次向某个自动化推送机器人发送消息后，客户端会按“机器人 + `chatid`”记录该群，并只把它加入这个机器人的“已发现群”下拉框；在群中 @该机器人发送 `/chatid` 可以同时确认 ID。企业微信智能机器人消息不提供群名称，因此客户端允许为已发现群设置只保存在本机的名称，未命名群显示缩短后的 `chatid`。群发现只简化选择，不授予调用权限。需要允许群成员远程触发任务时，仍要在“高级设置”中配置用户白名单。配置好目标群和用户白名单后，可以引用带 `[RPT-…]` 的机器人报告继续提问，也可以发送 `/run 任务名称`。只有消息到达任务绑定的同一个机器人、群与用户权限也匹配时才会路由。如果一个群对当前用户只开放了一个任务，普通 @提问也会路由到该任务。用户白名单留空时禁止群内触发，只有明确填写 `*` 才允许群内所有成员。
...
Claude Workspace 可以通过一台专用的 **Claude Code 管理机器人** 建立企业微信 WebSocket 长连接，在 Claude Code 真正等待输入时发送通知，并把你的回复精确写回对应的本地会话。它与自动化推送机器人分开配置，不承担网页信息推送。该功能不需要暴露公网回调地址。
...
> [!WARNING]
> 同一组 Bot ID 和 Secret 同时只能维持一个有效长连接。多台电脑或多个用户同时运行时，每个客户端都应配置独立机器人，否则新连接会断开旧连接。Claude Code 管理机器人和自动化推送机器人也不能复用同一个 Bot ID；客户端会在保存时拒绝这种冲突。
...
- 自动化任务、最多 20 个自动化机器人配置、最近 500 条运行记录和最近发现的 200 个“机器人 + 企业微信群”组合独立保存在 `%APPDATA%\Claude Workspace\automation.json`。其中包含提示词、收件人地址、企业微信目标与用户 ID、机器人本机名称与 Bot ID、已发现群的 `chatid`、本机名称和最近出现时间、结果和诊断信息，但不保存 MCP 配置内容。
...
- Claude Code 管理机器人和各个自动化推送机器人的 Secret 都通过 Electron `safeStorage` 使用操作系统凭据保护能力加密后保存，不会以明文传给界面层。
...
- 每个客户端配置一台 Claude Code 管理机器人，并可另外配置最多 20 台自动化推送机器人。已启用的自动化机器人会同时保持长连接；连接数量会相应增加，请只启用实际需要的机器人。
...
- 终端远程回复来自 Claude Code 管理机器人单聊。自动化推送机器人的群聊只用于自动化路由，并且必须同时匹配任务绑定机器人、群 `chatid` 和允许用户；它不会写入交互式终端。
