...
3. 填写五字段定时表达式 `分 时 日 月 星期`。例如 `0 9 * * 1-5` 表示按电脑本地时区在工作日 09:00 运行。
4. 填写工程内 MCP 配置的相对路径，并逐行列出本任务允许使用的 MCP 服务器名称。后台进程只会临时加载这些服务器。
5. 填写任务提示词、固定邮件收件人、企业微信投递目标和允许群内交互的 `userid`，然后保存。也可以先点击“立即运行”验证结果。

企业微信群的 `chatid` 可以通过在目标群中 @机器人并发送 `/chatid` 查询。配置好目标群和用户白名单后，可以引用带 `[RPT-…]` 的机器人报告继续提问，也可以发送 `/run 任务名称`。如果一个群对当前用户只开放了一个任务，普通 @提问也会路由到该任务。用户白名单留空时禁止群内触发，只有明确填写 `*` 才允许群内所有成员。
